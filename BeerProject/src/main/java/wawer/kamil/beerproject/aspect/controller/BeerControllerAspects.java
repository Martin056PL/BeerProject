...
public class BeerControllerAspects {
    ...
    @Before(value = "execution(* wawer.kamil.beerproject.controllers.BeerController.addBeerToBreweryByBreweryId(..)) && args(breweryId, beerRequest)", argNames = "breweryId, beerRequest")
    public void logAddNewBeer(Long breweryId, BeerRequest beerRequest) {
    ...
    @Before(value = "execution(* wawer.kamil.beerproject.controllers.BeerController.downloadImage(..)) && args(beerId)", argNames = "beerId")
    public void logDownloadImage(Long beerId) {
        log.debug("Endpoint address: 'brewery/{breweryId}/beer/{beerId}/image' with GET method, request parameter - beerId: {}", beerId);
    }

    @Before(value = "execution(* wawer.kamil.beerproject.controllers.BeerController.uploadImage(..)) && args(beerId, multipartFile)", argNames = "beerId, multipartFile")
    public void logUploadImage(Long beerId, MultipartFile multipartFile) {
        log.debug("Endpoint address: 'brewery/{breweryId}/beer/{beerId}/image' with POST method, request parameter - beerId: {}, multipartFile: name: {}, contentType: {} , size: {}",
                beerId,
